...
codocs:
  ...
  source_hash: sha256:8c72552f81dc63436b6e10c10c06e10739d95cddc2e6e2174cb46e893b3adcb4
  explicit_deps: []
...
- `TonemapReinhard()` — 逐分量 Reinhard tonemap `x/(1+x)`；整数类型每分量视为 [0,255]
- `LinearToSRGB()` — 逐分量 IEC 61966-2-1 sRGB gamma；复用同名标量自由函数；整数类型每分量视为 [0,255] 线性输入，返回 [0,255] sRGB

### Display Transform 自由函数（定义于 TVector 之前）
`TonemapReinhard(float/double/uint8_t)` 和 `LinearToSRGB(float/double/uint8_t)` 定义在 `TVector` 模板之前，供 TVector 成员复用，也可独立调用。
